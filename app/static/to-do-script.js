    ...
    if (e.which == 13 || e.keyCode == 13) {
      ...
      fetch('/add-todo', {
        ...
        let html = `
            ...
            <td><button type="button" class="btn btn-outline-warning btn-sml">Edit Task</button></td>
...
//
  ...
  function displayTodaysTasks(itemsList) {
    ...
    for (var i = 0; i < itemsList.length; i++) {
      ...
      <td><a href="${itemsList[i]['link']}" class="link" target="_blank" style="target-new: tab;">${itemsList[i]['task']}</a></td>
  ...
  function assignToMe(item_id) {
    ...
    fetch('/assign-item', {
      ...
      let html = `
          ...
          <td>${data['time']}</td>
          <td><a href="${data.link}" class="link" target="_blank" style="target-new: tab;">${data['task']}</a></td>
...
//
  ...
  function editUser(firstname, lastname, username, id) {
    ...
    let change_password_div = document.querySelector('.change-password-span')

    change_password_div.innerHTML = `
                        <input id="change_password" name="change_password" type="checkbox" value="y">
